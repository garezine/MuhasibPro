...
namespace MuhasibPro.Infrastructure.Infrastructure.Common.VirtualCollection;
...
abstract public partial class VirtualCollection<T> : IItemsRangeInfo, INotifyCollectionChanged
{
    ...
    protected async void LogException(string source, string action, Exception exception)
    {
        ...
        {
            await LogService.WriteAsync(LogType.Hata, source, action, exception.Message, exception.ToString());
        } else
        {
            await LogService.WriteAsync(LogType.Hata, source, action, exception);
        }
    }
