...
public class FirmaDetailsViewModel : GenericDetailsViewModel<FirmaModel>
{
    ...
    public async Task LoadAsync(FirmaDetailsArgs args)
    {
        ...
        else
        {
            ...
            {
                LogSistemException("Firma", "Firma Detay", ex);
            }
    ...
    protected async override Task<bool> SaveItemAsync(FirmaModel model)
    {
        try
        {
            ...
            LogSistemInformation("Firma", "Kayıt", "Firma başarıyla kaydedildi", $"Firma {model.Id} '{model.KisaUnvani}' başarıyla kaydedildi");
            return true;
        }
        catch (Exception ex)
        {
            StatusError($"Firma kaydedilirken hata oluştu: {ex.Message}");
            LogSistemException("Firma", "Kayıt", ex);
            return false;
        }
    }
    protected async override Task<bool> DeleteItemAsync(FirmaModel model)
    {
        try
        {
            ...
            LogSistemWarning("Firma", "Sil", "Firma silindi", $"Firma {model.Id} '{model.KisaUnvani}' silindi");
            return true;
        }
        catch (Exception ex)
        {
            StatusError($"Firma silinirken hata oluştu: {ex.Message}");
            LogSistemException("Firma", "Sil", ex);
            return false;
        }
    }
                ...
                switch (message)
                {
                    case "ItemChanged":
                        await ContextService.RunAsync(async () =>
                        {
                            ...
                            {
                                LogSistemException("Firma", "Değiştirilmiş", ex);
                            }
                        });
                        break;
                    case "ItemDeleted":
        ...
        if (current != null)
        {
            switch (message)
            {
                ...
                case "ItemRangesDeleted":
                    ...
                    {
                        LogSistemException("Firma", "Firma kaydı silinmiş!", ex);
                    }
                    break;
            }
        }
