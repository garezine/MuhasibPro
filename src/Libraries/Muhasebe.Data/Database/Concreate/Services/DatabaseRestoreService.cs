...
namespace Muhasebe.Data.Database.Concreate.Services
{
    ...
    public class DatabaseRestoreService : IDatabaseRestoreService
    {
        ...
        public async Task CreateOrUpdateSistemDatabaseAsync()
        {
            try
            {
                System.Diagnostics.Debug.WriteLine("[Database] Veritabanı oluşturma işlemi başladı...");

                using var context = new DesignTimeSistemDbContextFactory().CreateDbContext(null);

                // Connection string'i kontrol et
                var connectionString = context.Database.GetConnectionString();
                System.Diagnostics.Debug.WriteLine($"[Database] Connection String: {connectionString}");

                // System32 kontrolü
                if (connectionString?.Contains("System32", StringComparison.OrdinalIgnoreCase) == true)
                {
                    throw new InvalidOperationException("HATA: Veritabanı System32 dizininde oluşturulmaya çalışılıyor! Bu güvenlik riski oluşturur.");
                }

                // SQLite dosya yolunu çıkar ve dizini oluştur
                if (connectionString?.Contains("Data Source=") == true)
                {                    
                    var dbPath = connectionString.Replace("Data Source=", "").Trim();
                    var dbDirectory = ConfigurationDb.GetProjectDirectory();

                    System.Diagnostics.Debug.WriteLine($"[Database] Veritabanı dosyası: {dbPath}");
                    System.Diagnostics.Debug.WriteLine($"[Database] Veritabanı dizini: {dbDirectory}");
                    ...
                    if (!string.IsNullOrEmpty(dbDirectory))
                    {
                        if (!Directory.Exists(dbDirectory))
                        {
                            ...
                            System.Diagnostics.Debug.WriteLine($"[Database] Dizin oluşturuldu: {dbDirectory}");
                        }

                        // Dizine yazma yetkisi var mı kontrol et
                        try
                        {
                            var testFile = Path.Combine(dbDirectory, "test.tmp");
                            File.WriteAllText(testFile, "test");
                            File.Delete(testFile);
                            System.Diagnostics.Debug.WriteLine("[Database] Dizin yazma yetkisi OK.");
                        }
                        catch (Exception ex)
                        {
                            System.Diagnostics.Debug.WriteLine($"[Database] Dizin yazma yetkisi HATASI: {ex.Message}");
                            throw new UnauthorizedAccessException($"Veritabanı dizinine yazma yetkisi yok: {dbDirectory}");
                        }
                    }
                }
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[Database] {pendingMigrations.Count()} migration uygulanacak...");
                    await context.Database.MigrateAsync();
                    System.Diagnostics.Debug.WriteLine("[Database] Migration'lar başarıyla uygulandı.");
                }
                else
                {
                    var created = await context.Database.EnsureCreatedAsync();
                    System.Diagnostics.Debug.WriteLine(created ?
                        "[Database] Yeni veritabanı oluşturuldu." :
                        "[Database] Veritabanı zaten mevcut.");
                }

                System.Diagnostics.Debug.WriteLine("[Database] Veritabanı işlemi başarıyla tamamlandı.");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[Database] KRITIK HATA: {ex.Message}");
                System.Diagnostics.Debug.WriteLine($"[Database] Stack Trace: {ex.StackTrace}");
                throw;
            }
        }
    }
