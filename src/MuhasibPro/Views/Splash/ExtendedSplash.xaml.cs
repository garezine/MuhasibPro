...
namespace MuhasibPro.Views.Splash
{
    public sealed partial class ExtendedSplash : Page
    {
        ...
        private async Task StartApplicationAsync()
        {
            try
            {
                StatusMessages.Enqueue("Sistem baþlatýlýyor...");
                await Task.Delay(1000);
                StatusMessages.Enqueue("Veritabaný kontrol ediliyor...");
                await Startup.Instance.EnsureSistemDbAsync();
                await Task.Delay(1000);
                StatusMessages.Enqueue("Yapýlandýrma yükleniyor...");
                await Startup.Instance.ConfigureAsync();
                await Task.Delay(1000);
                StatusMessages.Enqueue("Servisler hazýrlanýyor...");
                await Task.Delay(1000);
                StatusMessages.Enqueue("Klasörler düzenleniyor...");
                await Task.Delay(800);
                StatusMessages.Enqueue("Hazýr!");
